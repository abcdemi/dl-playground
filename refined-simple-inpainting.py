...
# ==============================================================================
# MODEL ARCHITECTURE: REFINED AUTOENCODER
# This model is refined to use both global and local context in the decoder.
# ==============================================================================
class RefinedAutoencoder(nn.Module):
    def __init__(self, bottleneck_channels=64):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.Conv2d(32, bottleneck_channels, kernel_size=3, padding=1), nn.ReLU(True), nn.MaxPool2d(2),
        )
        
        # Decoder's first layer is changed to accept the concatenated tensor
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(bottleneck_channels + 3, 32, kernel_size=2, stride=2),
            nn.ReLU(True),
            nn.ConvTranspose2d(32, 3, kernel_size=2, stride=2),
            nn.Sigmoid(),
        )

    def forward(self, x, mask): # Model now takes the mask as an input
        # 1. Get global context from the encoder
        latent_code = self.encoder(x)
        
        # 2. Get local context by downsampling the masked input
        local_context = F.interpolate(x, size=latent_code.shape[2:], mode='bilinear', align_corners=False)

        # 3. Fuse global and local context
        fused_representation = torch.cat([latent_code, local_context], dim=1)
        
        # 4. Pass the refined representation to the decoder
...
# ==============================================================================
# STEP 2: TRAIN THE REFINED AUTOENCODER MODEL
# ==============================================================================
print("\n--- STEP 2: Starting Refined Autoencoder Training ---")
...
# Instantiate the RefinedAutoencoder model
inpainting_model = RefinedAutoencoder().to(device)
...
for epoch in range(num_epochs):
    ...
    for original_imgs, masks in progress_bar:
        ...
        # MODIFIED FORWARD PASS: Pass both image and mask to the model
...
print("\n\n--- FINAL EVALUATION REPORT (Refined Autoencoder) ---")
